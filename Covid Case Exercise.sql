...
WITH
    uk (DateRecorded, DailyCases, RANKING)
    AS
    (
SELECT
	cc.DateRecorded
	, SUM(cc.DailyCases)
	--, RANK() OVER (ORDER BY DAILYCASES DESC) 
FROM
	CovidCase cc
GROUP BY
	cc.DateRecorded
...
SELECT CC.Country
		, CC.DateRecorded
		, CC.DailyCases
		, RANK() OVER (PARTITION BY CC.COUNTRY ORDER BY CC.DAILYCASES DESC) AS COUNTRYRANK
